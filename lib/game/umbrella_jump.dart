...
class UmbrellaJump extends FlameGame
    with HasKeyboardHandlerComponents, HasCollisionDetection {
  UmbrellaJump({super.children});
  ...
  void update(double dt) {
    ...
    if (gameManager.isReady) {
      overlays.add('nameOverlay');
      return;
    }

    if (gameManager.isIntro) {
      overlays.add('mainMenuOverlay'); // 앱 실행시 초기화면
      return;
    }
  ...
  void outRank() {
    overlays.remove('rankOverlay');
    overlays.add('mainMenuOverlay');
  }

  void outName() {
    gameManager.state = GameState.intro;
    overlays.remove('nameOverlay');
  }
